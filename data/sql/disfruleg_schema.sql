...
-- Tabla para secuencia de IDs de factura
CREATE TABLE IF NOT EXISTS factura_sequence (
    id INT PRIMARY KEY DEFAULT 1,
    next_val INT NOT NULL DEFAULT 1
);

INSERT INTO factura_sequence (id, next_val) 
SELECT 1, COALESCE(MAX(id_factura), 0) + 1 FROM factura
ON DUPLICATE KEY UPDATE next_val = GREATEST(next_val, 1);

SELECT 'Tabla factura_sequence creada' AS status;
SELECT * FROM factura_sequence;
